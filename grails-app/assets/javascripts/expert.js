...
var customDepth = {
    ...
    init: function () {
        ...
        // bind listeners
        $('#minDepth, #maxDepth').on('change', function () {
            that.inputChange()
        });
        this.$bathome.on('change', function () {
            that.bathomeChange()
        });
    },
    ...
    setFromQuery: function (minDepth, maxDepth) {
        ...
        switch (depth) {
            case "undefined-40":
                $bathome.val('coastal/shallow water (0-40m)');
                this.bathomeChange();
                break;
            case "undefined-200":
                $bathome.val('shelf (0-200m)');
                this.bathomeChange();
                break;
            case "undefined-500":
                $bathome.val('shelf + upper slope (0-500m)');
                this.bathomeChange();
                break;
            case "200-500":
                $bathome.val('upper slope only (200-500m)');
                this.bathomeChange();
                break;
            case "undefined-undefined":
                $bathome.val('any (0-2000+m)');
                this.bathomeChange();
                break;
            default:
...
var locationWidgets = {
    ...
    init: function (initialRadius, localities) {
        ...
        // bind listeners
        this.$locality.on('change', function () {
            that.localityChange();
        });
        this.$imcra.on('change', function () {
            that.imcraChange();
        });
        //Added by Alan on for fetching multiple layers on 30/07/2014 --- START
        this.$myLayer.on('change', function () {
            that.myLayerChange();
        });
        //Added by Alan --- END
...
function search(headless) {
    ...
    $.post(serverUrl + "/search/ajaxSearch", $('form').serialize(), function (data) {
        ...
        $('#searchResults').animate({
            borderColor: '#DF4A21'/*'#f2bd99'*//*'#ff7154'*/,
            backgroundColor: '#f2bd99', color: '#DF4A21'
        }, 1000, function () {
            $('#searchResults').animate({
